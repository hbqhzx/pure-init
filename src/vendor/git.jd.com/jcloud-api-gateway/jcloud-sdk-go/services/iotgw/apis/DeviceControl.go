// Copyright 2018 JDCLOUD.COM
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
//
// NOTE: This class is auto generated by the jdcloud code generator program.

package apis

import (
    "git.jd.com/jcloud-api-gateway/jcloud-sdk-go/core"
    iotgw "git.jd.com/jcloud-api-gateway/jcloud-sdk-go/services/iotgw/models"
)

type DeviceControlRequest struct {

    core.JDCloudRequest

    /* regionId  */
    RegionId string `json:"regionId"`

    /* 实例ID  */
    InstanceId string `json:"instanceId"`

    /* iotgw实例下发设备控制指令  */
    Devicecmd *iotgw.DeviceControlSpec `json:"devicecmd"`
}

/*
 * param regionId: regionId (Required)
 * param instanceId: 实例ID (Required)
 * param devicecmd: iotgw实例下发设备控制指令 (Required)
 *
 * @Deprecated, not compatible when mandatory parameters changed
 */
func NewDeviceControlRequest(
    regionId string,
    instanceId string,
    devicecmd *iotgw.DeviceControlSpec,
) *DeviceControlRequest {

	return &DeviceControlRequest{
        JDCloudRequest: core.JDCloudRequest{
			URL:     "/regions/{regionId}/instances/{instanceId}:deviceControl",
			Method:  "POST",
			Header:  nil,
			Version: "v1",
		},
        RegionId: regionId,
        InstanceId: instanceId,
        Devicecmd: devicecmd,
	}
}

/*
 * param regionId: regionId (Required)
 * param instanceId: 实例ID (Required)
 * param devicecmd: iotgw实例下发设备控制指令 (Required)
 */
func NewDeviceControlRequestWithAllParams(
    regionId string,
    instanceId string,
    devicecmd *iotgw.DeviceControlSpec,
) *DeviceControlRequest {

    return &DeviceControlRequest{
        JDCloudRequest: core.JDCloudRequest{
            URL:     "/regions/{regionId}/instances/{instanceId}:deviceControl",
            Method:  "POST",
            Header:  nil,
            Version: "v1",
        },
        RegionId: regionId,
        InstanceId: instanceId,
        Devicecmd: devicecmd,
    }
}

/* This constructor has better compatible ability when API parameters changed */
func NewDeviceControlRequestWithoutParam() *DeviceControlRequest {

    return &DeviceControlRequest{
            JDCloudRequest: core.JDCloudRequest{
            URL:     "/regions/{regionId}/instances/{instanceId}:deviceControl",
            Method:  "POST",
            Header:  nil,
            Version: "v1",
        },
    }
}

/* param regionId: regionId(Required) */
func (r *DeviceControlRequest) SetRegionId(regionId string) {
    r.RegionId = regionId
}

/* param instanceId: 实例ID(Required) */
func (r *DeviceControlRequest) SetInstanceId(instanceId string) {
    r.InstanceId = instanceId
}

/* param devicecmd: iotgw实例下发设备控制指令(Required) */
func (r *DeviceControlRequest) SetDevicecmd(devicecmd *iotgw.DeviceControlSpec) {
    r.Devicecmd = devicecmd
}

// GetRegionId returns path parameter 'regionId' if exist,
// otherwise return empty string
func (r DeviceControlRequest) GetRegionId() string {
    return r.RegionId
}

type DeviceControlResponse struct {
    RequestID string `json:"requestId"`
    Error core.ErrorResponse `json:"error"`
    Result DeviceControlResult `json:"result"`
}

type DeviceControlResult struct {
}