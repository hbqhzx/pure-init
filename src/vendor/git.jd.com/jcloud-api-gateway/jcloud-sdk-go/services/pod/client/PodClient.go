...
import (
    ...
    pod "git.jd.com/jcloud-api-gateway/jcloud-sdk-go/services/pod/apis"
    "encoding/json"
    "errors"
)
 ...
 */
func (c *PodClient) ModifyQuota(request *pod.ModifyQuotaRequest) (*pod.ModifyQuotaResponse, error) {
    if request == nil {
        return nil, errors.New("Request object is nil. ")
    }
    resp, err := c.Send(request, c.ServiceName)
    ...
    jdResp := &pod.ModifyQuotaResponse{}
    err = json.Unmarshal(resp, jdResp)
    if err != nil {
        c.Logger.Log(core.LogError, "Unmarshal json failed, resp: %s", string(resp))
        return nil, err
