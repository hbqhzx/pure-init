 ...
 * param classPath: 作业路径 (Required)
 * param triggerName: 触发器名称 (Required)
 * param triggerGroup: 触发器组 (Required)
 * param cronExpression: 定时表达式 (Required)
 * param enable: 是否已启用，1：已启用 0：未启用 (Required)
 *
 ...
 * param cronExpression: 定时表达式 (Required)
 * param enable: 是否已启用，1：已启用 0：未启用 (Required)
 * param taskType: 定时任务类型(从接口中获取类型列表) (Optional)
...
/* param enable: 是否已启用，1：已启用 0：未启用(Required) */
func (r *CreateTaskRequest) SetEnable(enable int) {
    r.Enable = enable
}

/* param taskType: 定时任务类型(从接口中获取类型列表)(Optional) */
func (r *CreateTaskRequest) SetTaskType(taskType int) {
    r.TaskType = &taskType
}
