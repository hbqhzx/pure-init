...
type Backend struct {
    ...
    /* 后端服务所属负载均衡类型，取值为：alb、nlb (Optional) */
    LoadBalancerType string `json:"loadBalancerType"`

    /* 后端服务的协议【alb】包括Http，Tcp【nlb】包括Tcp (Optional) */
    Protocol string `json:"protocol"`

    /* 后端服务的端口，取值范围为[1, 65535] (Optional) */
    Port int `json:"port"`

    /* 调度算法, 取值为IpHash, RoundRobin, LeastConn，默认为RoundRobin (Optional) */
    Algorithm string `json:"algorithm"`

    /* 服务器组的Id列表，目前只支持一个，且与agIds不能同时存在 (Optional) */
    TargetGroupIds []string `json:"targetGroupIds"`

    /* 可用性组的Id列表，目前只支持一个，且与targetGroupIds不能同时存在 (Optional) */
    ...
    /* 【nlb】移除target前连接的最大保持时间，范围[1，3600] (Optional) */
    ConnectionDrainingSeconds int `json:"connectionDrainingSeconds"`

    /* 后端服务的后端保持, 取值为false(不开启)或者true(开启)【alb Http协议，RoundRobin算法】支持基于cookie的会话保持【nlb】支持基于报文源目的IP的会话保持 (Optional) */
