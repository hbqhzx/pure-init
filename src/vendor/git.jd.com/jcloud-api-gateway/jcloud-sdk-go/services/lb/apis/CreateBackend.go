...
type CreateBackendRequest struct {
    ...
    /* 调度算法, 取值范围为[IpHash, RoundRobin,和LeastConn]（取值范围的含义分别为：源Ip hash，加权轮询和加权最小连接），默认为RoundRobin（加权轮询） (Optional) */
 ...
 * param protocol: 后端服务的协议【alb】取值范围：Http、Tcp【nlb】取值范围：Tcp (Required)
 ...
 * param port: 后端服务的端口，取值范围为[1, 65535]，如指定了TargetSpec中的port，实际按照target指定的port进行转发 (Required)
 * param healthCheckSpec: 健康检查信息 (Required)
 * param algorithm: 调度算法, 取值范围为[IpHash, RoundRobin,和LeastConn]（取值范围的含义分别为：源Ip hash，加权轮询和加权最小连接），默认为RoundRobin（加权轮询） (Optional)
...
/* param proxyProtocolV2: 【nlb】是否启用Proxy ProtocolV2协议, 取值为false或者true, 默认为false(Optional) */
func (r *CreateBackendRequest) SetProxyProtocolV2(proxyProtocolV2 bool) {
