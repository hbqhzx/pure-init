...
import (
    ...
    kubernetes "git.jd.com/jcloud-api-gateway/jcloud-sdk-go/services/kubernetes/apis"
    "encoding/json"
    "errors"
)
...
/* 修改(k8s 集群)配额，内部接口 */
func (c *KubernetesClient) ModifyQuota(request *kubernetes.ModifyQuotaRequest) (*kubernetes.ModifyQuotaResponse, error) {
    if request == nil {
        return nil, errors.New("Request object is nil. ")
    }
    resp, err := c.Send(request, c.ServiceName)
    ...
    jdResp := &kubernetes.ModifyQuotaResponse{}
    err = json.Unmarshal(resp, jdResp)
    if err != nil {
        c.Logger.Log(core.LogError, "Unmarshal json failed, resp: %s", string(resp))
        return nil, err
