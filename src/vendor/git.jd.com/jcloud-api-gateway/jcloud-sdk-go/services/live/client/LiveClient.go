...
import (
    ...
    live "git.jd.com/jcloud-api-gateway/jcloud-sdk-go/services/live/apis"
    "encoding/json"
    "errors"
)
...
/* 查询直播录制时长数据 */
func (c *LiveClient) DescribeLiveRecordData(request *live.DescribeLiveRecordDataRequest) (*live.DescribeLiveRecordDataResponse, error) {
    if request == nil {
        return nil, errors.New("Request object is nil. ")
    }
    resp, err := c.Send(request, c.ServiceName)
    ...
    jdResp := &live.DescribeLiveRecordDataResponse{}
    err = json.Unmarshal(resp, jdResp)
    if err != nil {
        c.Logger.Log(core.LogError, "Unmarshal json failed, resp: %s", string(resp))
        return nil, err
