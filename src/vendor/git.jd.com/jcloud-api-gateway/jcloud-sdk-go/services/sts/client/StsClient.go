...
import (
    ...
    sts "git.jd.com/jcloud-api-gateway/jcloud-sdk-go/services/sts/apis"
    "encoding/json"
    "errors"
)
...
/* 服务角色与服务相关角色代入 */
func (c *StsClient) AssumeServiceRole(request *sts.AssumeServiceRoleRequest) (*sts.AssumeServiceRoleResponse, error) {
    if request == nil {
        return nil, errors.New("Request object is nil. ")
    }
    resp, err := c.Send(request, c.ServiceName)
    ...
    jdResp := &sts.AssumeServiceRoleResponse{}
    err = json.Unmarshal(resp, jdResp)
    if err != nil {
        c.Logger.Log(core.LogError, "Unmarshal json failed, resp: %s", string(resp))
        return nil, err
