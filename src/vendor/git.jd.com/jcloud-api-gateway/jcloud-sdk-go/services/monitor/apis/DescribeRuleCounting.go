...
filter name 为serviceCodes表示查询多个产品线的规则 (Optional) */
    ...
    /* groupBy,对于不同serviceCode是否分组，默认false,groupBy=true时仅在serviceCodes不为空时生效 (Optional) */
 ...
 * param groupBy: groupBy,对于不同serviceCode是否分组，默认false,groupBy=true时仅在serviceCodes不为空时生效 (Optional)
 */
...
/* param ruleType: ruleType,默认1(Optional) */
func (r *DescribeRuleCountingRequest) SetRuleType(ruleType int) {
    r.RuleType = &ruleType
}

/* param groupBy: groupBy,对于不同serviceCode是否分组，默认false,groupBy=true时仅在serviceCodes不为空时生效(Optional) */
func (r *DescribeRuleCountingRequest) SetGroupBy(groupBy bool) {
    r.GroupBy = &groupBy
}
...
type DescribeRuleCountingResult struct {
    ...
    RuleStats []monitor.RuleCounting `json:"ruleStats"`
