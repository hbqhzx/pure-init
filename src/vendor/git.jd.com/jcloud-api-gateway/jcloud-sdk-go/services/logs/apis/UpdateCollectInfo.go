...
type UpdateCollectInfoRequest struct {
    ...
    /* 采集实例类型, 只能是 all/part  当选择all时，传入的实例列表无效 (Optional) */
    ...
    /* 过滤器。设置过滤器后可根据用户设定的关键词采集部分日志，如仅采集 Error 的日志。目前最大允许20个。 (Optional) */
 ...
 * param collectInfoUID: 采集配置 UID (Required)
 * param enabled: 采集状态，0-禁用，1-启用 (Required)
 * param resourceType: 采集实例类型, 只能是 all/part  当选择all时，传入的实例列表无效 (Optional)
 ...
 * param logFilters: 过滤器。设置过滤器后可根据用户设定的关键词采集部分日志，如仅采集 Error 的日志。目前最大允许20个。 (Optional)
 */
