...
import (
    ...
    containerregistry "git.jd.com/jcloud-api-gateway/jcloud-sdk-go/services/containerregistry/apis"
    "encoding/json"
    "errors"
)
...
/* 修改配额（内部接口） */
func (c *ContainerregistryClient) ModifyQuota(request *containerregistry.ModifyQuotaRequest) (*containerregistry.ModifyQuotaResponse, error) {
    if request == nil {
        return nil, errors.New("Request object is nil. ")
    }
    resp, err := c.Send(request, c.ServiceName)
    ...
    jdResp := &containerregistry.ModifyQuotaResponse{}
    err = json.Unmarshal(resp, jdResp)
    if err != nil {
        c.Logger.Log(core.LogError, "Unmarshal json failed, resp: %s", string(resp))
        return nil, err
