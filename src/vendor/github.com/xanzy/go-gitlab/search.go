...
func (s *SearchService) UsersByProject(pid interface{}, query string, opt *SearchOptions, options ...OptionFunc) ([]*ProjectMember, *Response, error) {
	var pm []*ProjectMember
	resp, err := s.searchByProject(pid, "users", query, &pm, opt, options...)
	return pm, resp, err
}
